...
<Meta title="Atomic Design/Organisms/Navbar" />

# Navigation

Navbar represent bar that could be rendered top and at the bottom of the page. It has variable options to customize it.

## Color and opacity variations

There are two options how to colorize navigation. It is possible to use light by default or dark variant with opacity or without by default.

### Default

Shows the most simplified solution.

export const navbar = Builder()
  .dark(false)
  .primaryLinks([Builder()
    .title('Lorem')
    .link('')
    .build()
  ])
  .secondaryLinks([Builder()
      .title('Lorem')
      .link('')
      .build()
  ])
  .arePrimaryLinks(true)
  .onNavigate(() => {})
  .brandLink('/')
  .areButtons(false)
  .build();
...
<Canvas>
  <Story name="Default (Light)">
    <Navbar {...navbar} brand={<Render>

...
export const ProductDescription: FunctionComponent<ProductDescriptionProps> = ({
  ...
  <div className={cx('d-flex align-items-center justify-content-center h-100', className)}>
    <div className="d-flex flex-column w-100">
      <h3>{title}</h3>
      <Guard Component={Label}>
        {children}
      </Guard>
      <Guard Component={Description}>
        {children}
      </Guard>
      <div className="d-flex justify-content-between flex-row flex-sm-column justify-content-sm-center">
        ...
        <Guard<ImageProps> Component={Image} props={{ className: 'product-description-image' }}>
          {children}
        </Guard>
        <Guard Component={Button}>
          {children}
        </Guard>
      </div>
    </div>
